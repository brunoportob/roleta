...
export class Wheel {
  ...
  names.forEach((n,i)=>{
      ...
      // Posiciona texto mais interno para não encostar na borda
      const tr = r*0.62; // raio usado para texto
      ...
      // Cálculo de rotação: base radial + correção para manter legível (não invertido)
      const midDeg = (mid*180/Math.PI); // -90 já incorporado na fórmula acima
      let rot = (midDeg + 90) % 360;    // coloca texto tangencial ao raio
      if (rot < 0) rot += 360;
      // Se ficaria de cabeça para baixo (entre 90 e 270), gira 180° para ficar legível
      if (rot > 90 && rot < 270) rot = (rot + 180) % 360;
      text.setAttribute('transform', `rotate(${rot} ${tx} ${ty})`);

      // Exibição: placeholders aparecem como "Não foi dessa vez" na roda, mas internamente continuam nomeados "Nenhum item".
